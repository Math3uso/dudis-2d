...
Shader LoadShader(const char *vsFileName, const char *fsFileName)
{
    ...
    if ((vShaderStr == NULL) && (fShaderStr == NULL))
        TraceLog(LOG_WARNING, "SHADER: Shader files provided are not valid, using default shader");
...
void SetConfigFlags(unsigned int flags)
{
    if (CORE.Window.ready)
        TRACELOG(LOG_WARNING, "WINDOW: SetConfigFlags called after window initialization, Use \"SetWindowState\" to set flags instead");
    ...
    if (fileExt != NULL)
    {
        ...
        for (int i = 0; (i < fileExtLen) && (i < 16); i++)
        {
            ...
            if ((fileExt[i] >= 'A') && (fileExt[i] <= 'Z'))
                fileExtLower[i] = fileExt[i] + 32;
            else
                fileExtLower[i] = fileExt[i];
        }
    ...
    else
    {
        ...
        */
        ...
        if (raeFile != NULL)
        {
            ...
            if (result != buffer)
                TRACELOG(LOG_WARNING, "AUTOMATION: [%s] Issue reading line to buffer", fileName);
...
bool IsGamepadButtonPressed(int gamepad, int button)
{
    ...
    if ((gamepad < MAX_GAMEPADS) && CORE.Input.Gamepad.ready[gamepad] && (button < MAX_GAMEPAD_BUTTONS) &&
        (CORE.Input.Gamepad.previousButtonState[gamepad][button] == 0) && (CORE.Input.Gamepad.currentButtonState[gamepad][button] == 1))
        pressed = true;
...
bool IsGamepadButtonReleased(int gamepad, int button)
{
    ...
    if ((gamepad < MAX_GAMEPADS) && CORE.Input.Gamepad.ready[gamepad] && (button < MAX_GAMEPAD_BUTTONS) &&
        (CORE.Input.Gamepad.previousButtonState[gamepad][button] == 1) && (CORE.Input.Gamepad.currentButtonState[gamepad][button] == 0))
        released = true;
