 ...
 * Some of those formats are supported by default, to remove support, just
 * comment unrequired #define in this module
 *
 *       #define SUPPORT_IMAGE_EXPORT
 *           Support image export in multiple file formats
 *
 *       #define SUPPORT_IMAGE_MANIPULATION
 *           Support multiple image editing functions to scale, adjust colors,
 * flip, draw on images, crop... If not defined only some image editing
 * functions supported: ImageFormat(), ImageAlphaMask(), ImageResize*()
 *
...
// Image fileformats not supported by default
     ...
     defined(SUPPORT_FILEFORMAT_HDR) || defined(SUPPORT_FILEFORMAT_PIC) ||     \
...
#if (defined(SUPPORT_FILEFORMAT_DDS) || defined(SUPPORT_FILEFORMAT_PKM) ||     \
     defined(SUPPORT_FILEFORMAT_KTX) || defined(SUPPORT_FILEFORMAT_PVR) ||     \
...
extern void LoadFontDefault(
    void); // [Module: text] Loads default font, required by ImageDrawText()
...
static Vector4 *
LoadImageDataNormalized(Image image); // Load pixel data from image as Vector4
...
Image LoadImage(const char *fileName) {
  Image image = {0};
    ...
    defined(SUPPORT_FILEFORMAT_HDR) || defined(SUPPORT_FILEFORMAT_PNM) ||      \
...
Image LoadImageRaw(const char *fileName, int width, int height, int format,
                   int headerSize) {
  Image image = {0};
...
Image LoadImageAnim(const char *fileName, int *frames) {
  ...
  if (IsFileExtension(fileName, ".gif")) {
    ...
    if (fileData != NULL) {
      ...
      int *delays = NULL;
      image.data =
          stbi_load_gif_from_memory(fileData, dataSize, &delays, &image.width,
...
//  - All frames are returned in RGBA format
//  - Frames delay data is discarded
Image LoadImageAnimFromMemory(const char *fileType,
...
Image LoadImageFromMemory(const char *fileType, const unsigned char *fileData,
  ...
  if ((false)
    ...
    if (fileData != NULL) {
      int comp = 0;
      image.data = stbi_load_from_memory(fileData, dataSize, &image.width,
                                         &image.height, &comp, 0);
    ...
    if (fileData != NULL) {
      int comp = 0;
      image.data = stbi_loadf_from_memory(fileData, dataSize, &image.width,
                                          &image.height, &comp, 0);
    ...
    image.data =
        rl_load_dds_from_memory(fileData, dataSize, &image.width, &image.height,
    ...
    image.data =
        rl_load_pkm_from_memory(fileData, dataSize, &image.width, &image.height,
                                &image.format, &image.mipmaps);
  }
#endif
#if defined(SUPPORT_FILEFORMAT_KTX)
    ...
    image.data =
        rl_load_ktx_from_memory(fileData, dataSize, &image.width, &image.height,
                                &image.format, &image.mipmaps);
  }
#endif
#if defined(SUPPORT_FILEFORMAT_PVR)
    ...
    image.data =
        rl_load_pvr_from_memory(fileData, dataSize, &image.width, &image.height,
                                &image.format, &image.mipmaps);
  }
#endif
#if defined(SUPPORT_FILEFORMAT_ASTC)
    ...
    image.data =
        rl_load_astc_from_memory(fileData, dataSize, &image.width,
                                 &image.height, &image.format, &image.mipmaps);
  }
#endif
  else
  ...
  if (image.data != NULL)
    TRACELOG(LOG_INFO,
             ...
             image.width, image.height, rlGetPixelFormatName(image.format),
             image.mipmaps);
  else
...
Image LoadImageFromTexture(Texture2D texture) {
  ...
  if (texture.format < PIXELFORMAT_COMPRESSED_DXT1_RGB) {
    image.data = rlReadTexturePixels(texture.id, texture.width, texture.height,
                                     texture.format);
    ...
    if (image.data != NULL) {
      ...
      image.format = PIXELFORMAT_UNCOMPRESSED_R8G8B8A8;
#endif
      TRACELOG(LOG_INFO, "TEXTURE: [ID %i] Pixel data retrieved successfully",
               texture.id);
    } else
      TRACELOG(LOG_WARNING, "TEXTURE: [ID %i] Failed to retrieve pixel data",
               texture.id);
  } else
...
bool ExportImage(Image image, const char *fileName) {
  ...
  if (false) {
    ...
    unsigned char *fileData = stbi_write_png_to_mem(
        (const unsigned char *)imgData, image.width * channels, image.width,
    ...
    result = stbi_write_jpg(fileName, image.width, image.height, channels,
                            imgData, 90); // JPG quality: between 1 and 100
#endif
#if defined(SUPPORT_FILEFORMAT_QOI)
    ...
    if (image.format == PIXELFORMAT_UNCOMPRESSED_R8G8B8)
      ...
      TRACELOG(LOG_WARNING,
               "IMAGE: Image pixel format must be R8G8B8 or R8G8B8A8");
    ...
    result = rl_save_ktx(fileName, image.data, image.width, image.height,
                         image.format, image.mipmaps);
  }
#endif
    ...
    result =
        SaveFileData(fileName, image.data,
...
unsigned char *ExportImageToMemory(Image image, const char *fileType,
                                   int *dataSize) {
  ...
  *dataSize = 0;
  ...
  if ((strcmp(fileType, ".png") == 0) || (strcmp(fileType, ".PNG") == 0)) {
    fileData = stbi_write_png_to_mem((const unsigned char *)image.data,
                                     image.width * channels, image.width,
                                     image.height, channels, dataSize);
  }
#endif
...
#define TEXT_BYTES_PER_LINE 20
  ...
  byteCount += sprintf(txtData + byteCount,
                       ...
                       "                               //\n");
  byteCount += sprintf(txtData + byteCount,
                       "// ImageAsCode exporter v1.0 - Image pixel data "
  ...
  byteCount +=
      sprintf(txtData + byteCount, "static unsigned char %s_DATA[%i] = { ",
  ...
  if (success != 0)
    TRACELOG(LOG_INFO, "FILEIO: [%s] Image as code exported successfully",
             fileName);
  else
    TRACELOG(LOG_WARNING, "FILEIO: [%s] Failed to export image as code",
             fileName);
...
// amount
Image GenImageGradientLinear(int width, int height, int direction, Color start,
                             Color end) {
...
Image GenImageGradientRadial(int width, int height, float density, Color inner,
                             Color outer) {
  Color *pixels = (Color *)RL_MALLOC(width * height * sizeof(Color));
  ...
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      ...
      factor = (float)fmin(
          factor, 1.f); // dist can be bigger than radius, so we have to check
...
Image GenImageGradientSquare(int width, int height, float density, Color inner,
                             Color outer) {
  Color *pixels = (Color *)RL_MALLOC(width * height * sizeof(Color));
...
Image GenImagePerlinNoise(int width, int height, int offsetX, int offsetY,
                          float scale) {
  Color *pixels = (Color *)RL_MALLOC(width * height * sizeof(Color));
...
Image GenImageText(int width, int height, const char *text) {
  ...
  memcpy(image.data, text,
         (textLength > imageViewSize) ? imageViewSize : textLength);
...
void ImageCrop(Image *image, Rectangle crop) {
  ...
  if (image->mipmaps > 1)
    TRACELOG(LOG_WARNING,
             "Image manipulation only applied to base mipmap level");
  if (image->format >= PIXELFORMAT_COMPRESSED_DXT1_RGB)
    TRACELOG(LOG_WARNING,
             "Image manipulation not supported for compressed formats");
  else {
    int bytesPerPixel = GetPixelDataSize(1, 1, image->format);
...
void ImageFormat(Image *image, int newFormat) {
  ...
  if ((newFormat != 0) && (image->format != newFormat)) {
    if ((image->format < PIXELFORMAT_COMPRESSED_DXT1_RGB) &&
      ...
      RL_FREE(image->data); // WARNING! We loose mipmaps data --> Regenerated at
                            // the end...
      image->data = NULL;
      ...
      case PIXELFORMAT_UNCOMPRESSED_GRAYSCALE: {
        image->data = (unsigned char *)RL_MALLOC(image->width * image->height *
      ...
      case PIXELFORMAT_UNCOMPRESSED_R5G6B5: {
        image->data = (unsigned short *)RL_MALLOC(image->width * image->height *
      ...
      case PIXELFORMAT_UNCOMPRESSED_R32G32B32: {
        image->data = (float *)RL_MALLOC(image->width * image->height * 3 *
                                         sizeof(float));
      ...
      case PIXELFORMAT_UNCOMPRESSED_R32G32B32A32: {
        image->data = (float *)RL_MALLOC(image->width * image->height * 4 *
                                         sizeof(float));
      ...
      TRACELOG(LOG_WARNING,
               "IMAGE: Data format is compressed, can not be converted");
  }
}
...
Image ImageText(const char *text, int fontSize, Color color) {
  ...
  imText = ImageTextEx(GetFontDefault(), text, (float)fontSize, (float)spacing,
                       color); // WARNING: Module required: rtext
#else
  imText = GenImageColor(200, 60,
                         BLACK); // Generating placeholder black image rectangle
...
Image ImageTextEx(Font font, const char *text, float fontSize, float spacing,
  ...
  // font size
  Vector2 imSize = MeasureTextEx(font, text, (float)font.baseSize,
                                 spacing); // WARNING: Module required: rtext
  ...
  for (int i = 0; i < size;) {
    ...
    int codepoint = GetCodepointNext(
        &text[i], &codepointByteCount); // WARNING: Module required: rtext
    int index =
  ...
  if (textSize.y != imSize.y) {
    ...
    if (font.texture.id == GetFontDefault().texture.id)
      ImageResizeNN(&imText, (int)(imSize.x * scaleFactor),
                    (int)(imSize.y * scaleFactor));
    else
      ImageResize(&imText, (int)(imSize.x * scaleFactor),
                  (int)(imSize.y * scaleFactor));
  }
#else
  imText = GenImageColor(200, 60,
...
Image ImageFromChannel(Image image, int selectedChannel) {
  ...
  if (image.format == PIXELFORMAT_UNCOMPRESSED_GRAYSCALE ||
    ...
    if (selectedChannel > 0) {
      TRACELOG(LOG_WARNING,
               "This image has only 1 channel. Setting channel to it.");
      selectedChannel = 0;
    }
  } else if (image.format == PIXELFORMAT_UNCOMPRESSED_GRAY_ALPHA) {
    if (selectedChannel > 1) {
      TRACELOG(LOG_WARNING,
               "This image has only 2 channels. Setting channel to alpha.");
      selectedChannel = 1;
    }
    ...
    if (selectedChannel > 2) {
      TRACELOG(LOG_WARNING,
               "This image has only 3 channels. Setting channel to red.");
      selectedChannel = 0;
    }
  }
  ...
  if (selectedChannel > 3) {
    TRACELOG(LOG_WARNING, "ImageFromChannel supports channels 0 to 3 (rgba). "
                          "Setting channel to alpha.");
    selectedChannel = 3;
  }
  ...
  unsigned char *pixels =
      (unsigned char *)RL_CALLOC(image.width * image.height,
                                 sizeof(unsigned char)); // Values from 0 to 255

  if (image.format >= PIXELFORMAT_COMPRESSED_DXT1_RGB)
    TRACELOG(LOG_WARNING, "IMAGE: Pixel data retrieval not supported for "
...
void ImageResize(Image *image, int newWidth, int newHeight) {
  ...
  if ((image->format == PIXELFORMAT_UNCOMPRESSED_GRAYSCALE) ||
    ...
    switch (image->format) {
    case PIXELFORMAT_UNCOMPRESSED_GRAYSCALE:
      stbir_resize_uint8_linear((unsigned char *)image->data, image->width,
                                image->height, 0, output, newWidth, newHeight,
                                0, (stbir_pixel_layout)1);
      break;
    case PIXELFORMAT_UNCOMPRESSED_GRAY_ALPHA:
      stbir_resize_uint8_linear((unsigned char *)image->data, image->width,
                                image->height, 0, output, newWidth, newHeight,
                                0, (stbir_pixel_layout)2);
      break;
    case PIXELFORMAT_UNCOMPRESSED_R8G8B8:
      stbir_resize_uint8_linear((unsigned char *)image->data, image->width,
                                image->height, 0, output, newWidth, newHeight,
    ...
    // problem...
    stbir_resize_uint8_linear((unsigned char *)pixels, image->width,
                              image->height, 0, (unsigned char *)output,
                              newWidth, newHeight, 0, (stbir_pixel_layout)4);
...
void ImageResizeCanvas(Image *image, int newWidth, int newHeight, int offsetX,
  ...
  if (image->format >= PIXELFORMAT_COMPRESSED_DXT1_RGB)
    ...
    for (int y = 1; y < newHeight; y++) {
      memcpy(resizedData + y * newWidth * bytesPerPixel, resizedData,
...
void ImageToPOT(Image *image, Color fill) {
  ...
  // NOTE: Just add the required amount of pixels at the right and bottom sides
  // of image...
...
void ImageKernelConvolution(Image *image, const float *kernel, int kernelSize) {
  ...
  if (kernelWidth * kernelWidth != kernelSize) {
    TRACELOG(LOG_WARNING,
             "IMAGE: Convolution kernel must be square to be applied");
    return;
  }
...
void ImageMipmaps(Image *image) {
  ...
  int mipSize = GetPixelDataSize(mipWidth, mipHeight,
                                 image->format); // Image data size (in bytes)
  ...
  while ((mipWidth != 1) || (mipHeight != 1)) {
    ...
    TRACELOGD("IMAGE: Next mipmap level: %i x %i - current size %i", mipWidth,
              mipHeight, mipSize);

    mipCount++;
    mipSize += GetPixelDataSize(mipWidth, mipHeight,
                                image->format); // Add mipmap size (in bytes)
  }

  if (image->mipmaps < mipCount) {
    // Create second buffer and copy data manually to it
    void *temp = RL_CALLOC(mipSize, 1);
    memcpy(temp, image->data,
           GetPixelDataSize(image->width, image->height, image->format));
    RL_FREE(image->data);
    image->data = temp;
    ...
    for (int i = 1; i < mipCount; i++) {
      ...
      TRACELOGD("IMAGE: Generating mipmap level: %i (%i x %i) - size: %i - "
                "offset: 0x%x",
                i, mipWidth, mipHeight, mipSize, nextmip);
      ImageResize(&imCopy, mipWidth,
                  mipHeight); // Uses internally Mitchell cubic downscale filter
...
void ImageDither(Image *image, int rBpp, int gBpp, int bBpp, int aBpp) {
  ...
  if ((rBpp + gBpp + bBpp + aBpp) > 16) {
    TRACELOG(LOG_WARNING,
             "IMAGE: Unsupported dithering bpps (%ibpp), only 16bpp or lower "
             "modes supported",
             (rBpp + gBpp + bBpp + aBpp));
  } else {
    Color *pixels = LoadImageColors(*image);
    ...
    if ((image->format != PIXELFORMAT_UNCOMPRESSED_R8G8B8) &&
        (image->format != PIXELFORMAT_UNCOMPRESSED_R8G8B8A8)) {
      TRACELOG(LOG_WARNING, "IMAGE: Format is already 16bpp or lower, "
    ...
    unsigned short rPixel, gPixel, bPixel,
        aPixel; // Used for 16bit pixel composition
...
#define MIN(a, b) (((a) < (b)) ? (a) : (b))
    ...
    for (int y = 0; y < image->height; y++) {
      for (int x = 0; x < image->width; x++) {
        ...
        // number of bits! We want to know how much color precision we have
        // lost...
        ...
        if (x < (image->width - 1)) {
          pixels[y * image->width + x + 1].r =
              MIN((int)pixels[y * image->width + x + 1].r +
                      (int)((float)rError * 7.0f / 16),
                  0xff);
          pixels[y * image->width + x + 1].g =
              MIN((int)pixels[y * image->width + x + 1].g +
                      (int)((float)gError * 7.0f / 16),
                  0xff);
          pixels[y * image->width + x + 1].b =
              MIN((int)pixels[y * image->width + x + 1].b +
        ...
        if ((x > 0) && (y < (image->height - 1))) {
          pixels[(y + 1) * image->width + x - 1].r =
              MIN((int)pixels[(y + 1) * image->width + x - 1].r +
                      (int)((float)rError * 3.0f / 16),
                  0xff);
          pixels[(y + 1) * image->width + x - 1].g =
              MIN((int)pixels[(y + 1) * image->width + x - 1].g +
                      (int)((float)gError * 3.0f / 16),
                  0xff);
          pixels[(y + 1) * image->width + x - 1].b =
              MIN((int)pixels[(y + 1) * image->width + x - 1].b +
        ...
        if (y < (image->height - 1)) {
          pixels[(y + 1) * image->width + x].r =
              MIN((int)pixels[(y + 1) * image->width + x].r +
                      (int)((float)rError * 5.0f / 16),
                  0xff);
          pixels[(y + 1) * image->width + x].g =
              MIN((int)pixels[(y + 1) * image->width + x].g +
                      (int)((float)gError * 5.0f / 16),
                  0xff);
          pixels[(y + 1) * image->width + x].b =
              MIN((int)pixels[(y + 1) * image->width + x].b +
...
void ImageFlipVertical(Image *image) {
  ...
  if (image->format >= PIXELFORMAT_COMPRESSED_DXT1_RGB)
    ...
    int bytesPerPixel = GetPixelDataSize(1, 1, image->format);
    unsigned char *flippedData = (unsigned char *)RL_MALLOC(
...
void ImageRotateCW(Image *image) {
  ...
  if (image->format >= PIXELFORMAT_COMPRESSED_DXT1_RGB)
    ...
    int bytesPerPixel = GetPixelDataSize(1, 1, image->format);
    unsigned char *rotatedData = (unsigned char *)RL_MALLOC(
        image->width * image->height * bytesPerPixel);
...
Color *LoadImageColors(Image image) {
  ...
  Color *pixels =
      (Color *)RL_MALLOC(image.width * image.height * sizeof(Color));
      ...
      if (pixels[i].a > 0) {
        ...
        if (!colorInPalette) {
          ...
          if (palCount >= maxPaletteSize) {
            ...
            TRACELOG(LOG_WARNING, "IMAGE: Palette is greater than %i colors",
                     maxPaletteSize);
          }
        }
      }
...
Color GetImageColor(Image image, int x, int y) {
  ...
  if ((x >= 0) && (x < image.width) && (y >= 0) && (y < image.height)) {
    ...
    case PIXELFORMAT_UNCOMPRESSED_R16: {
      color.r =
          (unsigned char)(HalfToFloat(((unsigned short *)
    ...
    case PIXELFORMAT_UNCOMPRESSED_R16G16B16: {
      color.r =
          (unsigned char)(HalfToFloat(((unsigned short *)image
                                           .data)[(y * image.width + x) * 3]) *
                          255.0f);
      color.g =
...
void ImageDrawPixel(Image *dst, int x, int y, Color color) {
  ...
  case PIXELFORMAT_UNCOMPRESSED_GRAYSCALE: {
    ...
    Vector3 coln = {(float)color.r / 255.0f, (float)color.g / 255.0f,
  ...
  case PIXELFORMAT_UNCOMPRESSED_R5G5B5A1: {
    ...
    Vector4 coln = {(float)color.r / 255.0f, (float)color.g / 255.0f,
...
void ImageDrawLine(Image *dst, int startPosX, int startPosY, int endPosX,
...
void ImageDrawCircle(Image *dst, int centerX, int centerY, int radius,
                     Color color) {
  int x = 0;
  int y = radius;
...
void ImageDrawCircleLines(Image *dst, int centerX, int centerY, int radius,
                          Color color) {
  int x = 0;
  int y = radius;
  int decesionParameter = 3 - 2 * radius;
...
void ImageDrawCircleLinesV(Image *dst, Vector2 center, int radius,
                           Color color) {
  ImageDrawCircleLines(dst, (int)center.x, (int)center.y, radius, color);
}

// Draw rectangle within an image
void ImageDrawRectangle(Image *dst, int posX, int posY, int width, int height,
                        Color color) {
  ImageDrawRectangleRec(
      dst, (Rectangle){(float)posX, (float)posY, (float)width, (float)height},
      color);
}

// Draw rectangle within an image (Vector version)
void ImageDrawRectangleV(Image *dst, Vector2 position, Vector2 size,
                         Color color) {
  ImageDrawRectangle(dst, (int)position.x, (int)position.y, (int)size.x,
                     (int)size.y, color);
}
...
void ImageDrawRectangleRec(Image *dst, Rectangle rec, Color color) {
  ...
  for (int y = 1; y < (int)rec.height; y++) {
    memcpy(pSrcPixel + (y * dst->width) * bytesPerPixel, pSrcPixel,
           bytesPerRow);
  }
}

// Draw rectangle lines within an image
void ImageDrawRectangleLines(Image *dst, Rectangle rec, int thick,
                             Color color) {
  ImageDrawRectangle(dst, (int)rec.x, (int)rec.y, (int)rec.width, thick, color);
  ImageDrawRectangle(dst, (int)rec.x, (int)(rec.y + thick), thick,
                     (int)(rec.height - thick * 2), color);
  ImageDrawRectangle(dst, (int)(rec.x + rec.width - thick),
                     (int)(rec.y + thick), thick, (int)(rec.height - thick * 2),
                     color);
  ImageDrawRectangle(dst, (int)rec.x, (int)(rec.y + rec.height - thick),
                     (int)rec.width, thick, color);
}

// Draw triangle within an image
void ImageDrawTriangle(Image *dst, Vector2 v1, Vector2 v2, Vector2 v3,
                       Color color) {
...
void ImageDrawTriangleEx(Image *dst, Vector2 v1, Vector2 v2, Vector2 v3,
  ...
  // normalization NOTE 1: Here, we act as if we multiply by 255 the reciprocal,
  // which avoids additional
  //         calculations in the loop. This is acceptable because we are only
...
void ImageDrawTriangleLines(Image *dst, Vector2 v1, Vector2 v2, Vector2 v3,
                            Color color) {
...
// center)
void ImageDrawTriangleFan(Image *dst, const Vector2 *points, int pointCount,
                          Color color) {
  if (pointCount >= 3) {
...
void ImageDrawTriangleStrip(Image *dst, const Vector2 *points, int pointCount,
                            Color color) {
  if (pointCount >= 3) {
...
void ImageDraw(Image *dst, Image src, Rectangle srcRec, Rectangle dstRec,
  ...
  if (dst->format >= PIXELFORMAT_COMPRESSED_DXT1_RGB)
    ...
    // In that case, we make a copy of source, and we apply all required
    // transform
    ...
    //    ColorAlphaBlend() [ ] Support f32bit channels drawing

    // TODO: Support PIXELFORMAT_UNCOMPRESSED_R32G32B32A32 and
    // PIXELFORMAT_UNCOMPRESSED_R1616B16A16
...
void ImageDrawText(Image *dst, const char *text, int posX, int posY,
  ...
  ImageDrawTextEx(dst, GetFontDefault(), text, position, (float)fontSize, 1.0f,
                  color); // WARNING: Module required: rtext
#else
...
void ImageDrawTextEx(Image *dst, Font font, const char *text, Vector2 position,
  ...
  Rectangle dstRec = {position.x, position.y, (float)imText.width,
                      (float)imText.height};
...
TextureCubemap LoadTextureCubemap(Image image, int layout) {
  ...
  if (layout != CUBEMAP_LAYOUT_AUTO_DETECT) {
    ...
    /*else if (layout == CUBEMAP_LAYOUT_PANORAMA)
    {
      ...
      for (int i = 0; i < 6; i++)
        ImageDraw(&faces, mipmapped, faceRecs[i],
                  (Rectangle){0, (float)size * i, (float)size, (float)size},
                  WHITE);
...
void UnloadTexture(Texture2D texture) {
  if (texture.id > 0) {
    ...
    TRACELOG(LOG_INFO, "TEXTURE: [ID %i] Unloaded texture data from VRAM (GPU)",
             texture.id);
  }
}
...
bool IsRenderTextureValid(RenderTexture2D target) {
  ...
  if ((target.id > 0) && // Validate OpenGL id (loaded on GPU)
      IsTextureValid(
          target.depth) && // Validate FBO depth texture/renderbuffer attachment
...
void UpdateTexture(Texture2D texture, const void *pixels) {
  rlUpdateTexture(texture.id, 0, 0, texture.width, texture.height,
...
void UpdateTextureRec(Texture2D texture, Rectangle rec, const void *pixels) {
  rlUpdateTexture(texture.id, (int)rec.x, (int)rec.y, (int)rec.width,
                  (int)rec.height, texture.format, pixels);
}
...
void GenTextureMipmaps(Texture2D *texture) {
  ...
  rlGenTextureMipmaps(texture->id, texture->width, texture->height,
...
void SetTextureFilter(Texture2D texture, int filter) {
  ...
  case TEXTURE_FILTER_TRILINEAR: {
    if (texture.mipmaps > 1) {
      ...
      TRACELOG(LOG_WARNING,
               "TEXTURE: [ID %i] No mipmaps available for TRILINEAR texture "
               "filtering",
               texture.id);
...
void SetTextureWrap(Texture2D texture, int wrap) {
  ...
  case TEXTURE_WRAP_MIRROR_REPEAT: {
    rlTextureParameters(texture.id, RL_TEXTURE_WRAP_S,
                        RL_TEXTURE_WRAP_MIRROR_REPEAT);
    rlTextureParameters(texture.id, RL_TEXTURE_WRAP_T,
                        RL_TEXTURE_WRAP_MIRROR_REPEAT);
  } break;
...
void DrawTextureRec(Texture2D texture, Rectangle source, Vector2 position,
                    Color tint) {
  Rectangle dest = {position.x, position.y, fabsf(source.width),
                    fabsf(source.height)};
  Vector2 origin = {0.0f, 0.0f};
...
void DrawTexturePro(Texture2D texture, Rectangle source, Rectangle dest,
  ...
  if (texture.id > 0) {
    ...
    if (rotation == 0.0f) {
      ...
      bottomRight.x = x + (dx + dest.width) * cosRotation -
                      (dy + dest.height) * sinRotation;
      bottomRight.y = y + (dx + dest.width) * sinRotation +
                      (dy + dest.height) * cosRotation;
    }
    ...
    if (flipX)
      rlTexCoord2f((source.x + source.width) / width,
  ...
  if (texture.id > 0) {
    ...
    if (nPatchInfo.layout == NPATCH_NINE_PATCH) {
      ...
      if (drawCenter) {
        ...
        rlVertex2f(vertC.x,
                   vertB.y); // Bottom-right corner for texture and quad
      ...
      if (drawMiddle) {
        ...
        rlVertex2f(vertB.x,
                   vertC.y); // Bottom-right corner for texture and quad
        rlTexCoord2f(coordB.x, coordB.y);
        ...
        if (drawCenter) {
          ...
          rlTexCoord2f(coordB.x, coordC.y);
          rlVertex2f(vertB.x,
                     vertC.y); // Bottom-left corner for texture and quad
      ...
      if (drawCenter) {
        ...
        rlVertex2f(vertC.x,
                   vertD.y); // Bottom-right corner for texture and quad
        rlTexCoord2f(coordC.x, coordC.y);
...
int ColorToInt(Color color) {
  ...
  result = (int)(((unsigned int)color.r << 24) | ((unsigned int)color.g << 16) |
...
Vector3 ColorToHSV(Color color) {
  Vector3 hsv = {0};
  Vector3 rgb = {(float)color.r / 255.0f, (float)color.g / 255.0f,
                 (float)color.b / 255.0f};
  float min, max, delta;
...
#define COLORALPHABLEND_INTEGERS
  ...
  if (src.a == 0)
    ...
    unsigned int alpha =
        (unsigned int)src.a + 1; // We are shifting by 8 (dividing by 256), so
                                 // we need to take that excess into account
...
void SetPixelColor(void *dstPtr, Color color, int format) {
  ...
  case PIXELFORMAT_UNCOMPRESSED_R5G5B5A1: {
    ...
    ((unsigned short *)dstPtr)[0] = (unsigned short)r << 11 |
                                    (unsigned short)g << 6 |
                                    (unsigned short)b << 1 | (unsigned short)a;
  ...
  case PIXELFORMAT_UNCOMPRESSED_R4G4B4A4: {
    ...
    ((unsigned short *)dstPtr)[0] = (unsigned short)r << 12 |
                                    (unsigned short)g << 8 |
                                    (unsigned short)b << 4 | (unsigned short)a;
...
static unsigned short FloatToHalf(float x) {
  ...
  const unsigned int b = uni.ui + 0x00001000; // Round-to-nearest-even: add last
