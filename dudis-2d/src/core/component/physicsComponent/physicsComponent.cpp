...
// shared_ptr<PhysicsComponent> PhysicsComponent::create(Entity *ownerModel) {

//   // auto physicsWorld = App::getPhysicsWorld();

//   auto physicsWorld = App::getCurrentWolrd();
...
//   auto ownerPos = owner->getGlobalPos();

//   bodyDef.position.Set(ownerPos.x * METERS_TO_PIXELS,
//                        ownerPos.y * METERS_TO_PIXELS);

//   // bodyDef.position.Set(ownerPos.x * PIXELS_TO_METERS,
//   //                      ownerPos.y * PIXELS_TO_METERS);

//   bodyDef.fixedRotation = !rotate;

//   this->body = world->CreateBody(&bodyDef);
//   App::setBody(world, body);
...
// PhysicsComponent::createBoxPhysics(Entity *owner, bool isStatic, bool rotate,
//                                    SizeI sizeBox) {
//   {
//     auto physicsWorld = App::getCurrentWolrd();
...
//     auto ownerSize = owner->getGlobalSize();

//     sizeBox.w = sizeBox.w == 0 ? ownerSize.w : sizeBox.w;
//     sizeBox.h = sizeBox.h == 0 ? ownerSize.h : sizeBox.h;
...
//   owner->rotate(body->GetAngle() * RAD2DEG);
// }
...
//   body->SetTransform(b2Vec2(owner->getGlobalPos().x * METERS_TO_PIXELS,
//                             owner->getGlobalPos().y * METERS_TO_PIXELS),
