...
//     SHADER_UNIFORM_VEC2); SetShaderValue(*shader, *timec, timeValue,
...
//     DrawTextureRec(bg->texture, (Rectangle){0, 0, 800, -600}, (Vector2){0,
//     0},
//                    WHITE);
...
void Scene2::start() {
  ...
  App::SceneDraw([]() {
    if (GuiButton(Rectangle{10, 10, 100, 20}, "voltar")) {
      auto scene = Scene::create();
      App::getSceneManager().setScene(move(scene));
      // auto scene1 = Scene::create();
      // App::getSceneManager().setScene(move(scene1));
    }
  });
}
